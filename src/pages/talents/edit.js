...
function TalentsEdit() {
  ...
  const fetchOneCategories = async () => {
    const res = await getData(`/cms/talent/${talentId}`);
  ...
  const uploadImage = async (file) => {
    ...
    if (e.target.name === "avatar") {
      if (
        ...
        let size = parseFloat(e?.target?.files[0]?.size / 3000000).toFixed(2);
  ...
  const handleSubmit = async () => {
    ...
    const res = await putData(`/cms/talent/${talentId}`, payload);
    if (res?.data?.data) {
      dispatch(
